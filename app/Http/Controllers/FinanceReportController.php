    ...
    public function overview(Request $request)
{
    ...
    if ($request->filled('filter_type') && $request->filter_type === 'expense') {
        $miscPaymentsQuery->whereRaw('1 = 0');
    }
    ...
    if ($request->filled('filter_date_from')) {
        $salaryQuery->whereDate('processed_at', '>=', $request->filter_date_from);
    }
    if ($request->filled('filter_date_to')) {
        $salaryQuery->whereDate('processed_at', '<=', $request->filter_date_to);
    }
    if ($request->filled('filter_section')) {
    ...
    // Merge all into a plain base Collection using concat()
    $allTransactions = collect()
        ->concat($incomeStudent)
        ->concat($incomeMisc)
        ->concat($expenseSalary)
        ->concat($expenseOther)
        ->sortByDesc('date')
        ->values();
    ...
    return view('finance_overview', compact(
        'paginated', 'filters', 'sections', 'types',
