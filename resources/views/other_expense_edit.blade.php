                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('other.expense.update', $otherExpense->id) }}" method="POST" class="px-2">
                                    ...
                                    <div class="form-card">
                                                    ...
                                                    <select name="session_id" id="session_id"
                                                        ...
                                                        @foreach($sessions as $session)
                                                            <option value="{{ $session->id }}"
                                                                {{ old('session_id', $otherExpense->session_id) == $session->id ? 'selected' : '' }}>
                                                                {{ $session->name }}
                                                    ...
                                                    <select name="term_id" id="term_id"
                                                        ...
                                                        @foreach($terms as $term)
                                                            <option value="{{ $term->id }}"
                                                                {{ old('term_id', $otherExpense->term_id) == $term->id ? 'selected' : '' }}>
                                                                {{ $term->name }}
                                                    ...
                                                    <select name="section_id"
                                                        ...
                                                        @foreach($sections as $section)
                                                            <option value="{{ $section->id }}"
                                                                {{ old('section_id', $otherExpense->section_id) == $section->id ? 'selected' : '' }}>
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <textarea name="description"
                                                        ...
                                                        rows="3" required>{{ old('description', $otherExpense->description) }}</textarea>
                                                    @error('description')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                        ...
                                        {{-- Recorded by (read-only info) --}}
                                        @if($otherExpense->createdBy)
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Recorded By</label>
                                                    <input type="text" class="form-control"
                                                        value="{{ $otherExpense->createdBy->name }}" readonly disabled>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control"
                                                        value="{{ $otherExpense->created_at->format('d M Y, h:i A') }}" readonly disabled>
                                                </div>
                                            </div>
                                        </div>
                                        @endif
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#session_id').on('change', function () {
                ...
                const currentTermId = {{ $otherExpense->term_id ?? 'null' }};
                ...
                $.get('/other-expense/terms/' + sessionId, function (terms) {
                    ...
                    terms.forEach(function (term) {
                        ...
                        const selected = (term.id == currentTermId) ? 'selected' : '';
