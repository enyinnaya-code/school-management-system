...
<body>
                            ...
                            <div class="card-body">
                                ...
                                <form method="POST" action="{{ route('result_sheets.update', $template->id) }}"
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="form-group mb-0">
                                                <label class="font-weight-bold">Rating Columns <span
                                                        class="text-danger">*</span></label>
                                                <small class="text-muted d-block mb-2">These become the column headers
                                                    on the printed sheet.</small>
                                    ...
                                    <div class="card mb-4 border-left border-success"
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    {{--
                                                    Term select — server-rendered from $terms collection.
                                                    Each term has {id, name}. We submit the name as term_name.
                                                    Falls back to a text input if $terms is empty (all names are NULL).
                                                    --}}
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">
                                                            Term <span class="text-danger">*</span>
                                                            <small class="text-muted font-weight-normal ml-1">(applies
                                                                to all sessions)</small>
                                                        </label>
                                                        @php $savedTermName = old('term_name', $template->term_name ??
                                                        ''); @endphp
                                                        <select name="term_name" id="termSelect"
                                                            ...
                                                            <option value="{{ $termName }}" {{
                                                                $savedTermName===$termName ? 'selected' : '' }}>
                                                                {{ $termName }}
                                                            </option>
                                                            @endforeach
                                                        </select>
                                                        ...
                                                        <small class="text-muted">
                                                            ...
                                                            Works for the selected term across <strong>every</strong>
                                                            academic session.
                                                        </small>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="form-group">
                                                <label class="font-weight-bold">Applicable Classes <span
                                                        class="text-danger">*</span></label>
                                                <small class="text-muted d-block mb-2">Subjects will be re-fetched when
                                                    you reload below.</small>
                                    ...
                                    <div class="card mb-4 border-left border-warning"
                                        ...
                                        <div
                                            class="card-header bg-warning py-2 d-flex justify-content-between align-items-center">
                                            <span>
                                                <i class="fas fa-book mr-1"></i>
                                                <strong>Step 3 — Build Sheet Structure</strong>
                                                <small class="ml-2 text-dark font-italic">
                                                    Drag <i class="fas fa-grip-vertical"></i> to reorder · Click Edit to
                                                    modify
                                                </small>
                                            </span>
    ...
    <style>
        ...
        .subject-row.has-data {
            border-color: #28a745;
        }

        .subject-row.active.has-data {
            border-color: #1a7a35;
            background: #e6f4ea;
        }

        .subject-row.sortable-ghost {
            opacity: .4;
            background: #f0f4ff;
        }

        .subject-row.sortable-drag {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('sectionSelect').addEventListener('change', function () {
        loadClasses(this.value, []); resetSubjectArea();
    });
...
function loadClasses(sectionId, preChecked) {
    ...
    if (!sectionId) { container.innerHTML='<span class="text-muted small">Select a section above.</span>'; document.getElementById('loadSubjectsBtn').disabled=true; return; }
    ...
    fetch(`/api/sections/${sectionId}/classes`).then(r=>r.json()).then(data => {
        ...
        if (!classes.length) { container.innerHTML='<span class="text-muted">No classes in this section.</span>'; return; }
        ...
        classes.forEach(cls => {
            ...
            html += `<div class="col-md-3 col-6"><div class="custom-control custom-checkbox mb-1">
                <input type="checkbox" class="custom-control-input class-checkbox" id="cls_${cls.id}" name="applicable_classes[]" value="${cls.id}" ${chk}>
...
function resetSubjectArea() { document.getElementById('subjectBuilderArea').innerHTML = ''; }

function loadSubjectsFromClasses() {
    const classIds = [...document.querySelectorAll('.class-checkbox:checked')].map(c=>c.value);
    if (!classIds.length) return;
    document.getElementById('sortableSubjectList').innerHTML = '<span class="text-muted small"><i class="fas fa-spinner fa-spin"></i> Loading...</span>';
...
function addSubtopic(courseId) {
    ...
    renderSubtopicDOM(courseId,idx); afterChange(courseId);
}
function renderSubtopicDOM(courseId,stIdx) {
    ...
    div.innerHTML=`<div class="card-header bg-white py-1 d-flex align-items-center justify-content-between">
        <div class="d-flex align-items-center flex-grow-1 mr-2" style="gap:6px">
            ...
            <input type="text" class="form-control form-control-sm st-name-input" placeholder="Sub-topic name" value="${escHtml(st.name)}"
    ...
    <div class="card-body py-2">
        ...
        <div class="input-group mt-2" style="max-width:640px">
            <input type="text" class="form-control form-control-sm" placeholder="Type skill text then Enter or Add" id="newItem_${courseId}_${stIdx}"
...
function addItem(courseId,stIdx) {
    ...
    document.getElementById(`itemsArea_${courseId}_${stIdx}`).insertAdjacentHTML('beforeend',itemHtml(courseId,stIdx,iIdx,text));
...
function scrapeBuilderIntoStore() {
    document.querySelectorAll('.st-label-input,.st-name-input').forEach(input => {
        const courseId=String(input.dataset.course); const stIdx=parseInt(input.dataset.idx,10); const field=input.dataset.field;
...
function validateForm(e) {
    ...
    if(!payload.length) { e.preventDefault(); return alert('No subject data found. Please click a subject row to confirm your structure is loaded.'); }
}
    </script>
</body>