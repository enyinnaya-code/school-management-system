{{-- resources/views/edit_fee_prospectus_details.blade.php --}}
        ...
        <div class="main-wrapper main-wrapper-1">
            ...
            <div class="main-content pt-5 mt-5">
                <section class="section mb-5 pb-1 px-0">
                    <div class="col-12">
                        <div class="card">
                            <form action="{{ route('fee.prospectus.update', Crypt::encrypt($prospectus->id)) }}" method="POST" class="needs-validation" novalidate>
                                @csrf
                                @method('PUT')
                                ...
                                <div class="card-header">
                                    <h4>Edit Fee Prospectus for {{ $class->name }} - {{ $section->section_name }} ({{ $term->name }})</h4>
                                </div>
                                <div class="card-body">
                                    <div id="fees-container">
                                        @if(count($fees) > 0)
                                            ...
                                            <div class="row mb-3 fee-row">
                                                <div class="col-md-5">
                                                    <input type="text" class="form-control" name="fees[{{ $index }}][item]" value="{{ $fee['item'] }}" required>
                                                </div>
                                                <div class="col-md-5">
                                                    <input type="number" class="form-control amount-input" name="fees[{{ $index }}][amount]" value="{{ $fee['amount'] }}" min="0" step="0.01" required>
                                                </div>
                                                <div class="col-md-2">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-2">
                                            <strong>Total: ₦<span id="total-amount">{{ number_format($prospectus->total_amount, 2) }}</span></strong>
                                        </div>
                                    </div>
                                    <!-- Submit Button -->
                                    <div class="form-group mt-4 pt-4">
                                        <button type="submit" class="btn btn-primary">
                                            <i class="fas fa-save"></i> Update Prospectus
                                        </button>
                                        <a href="{{ route('fee.prospectus.manage') }}" class="btn btn-light">Back</a>
                                    </div>
                                </div>
                            </form>
                        </div>
